...
import type { TransportProtocol } from '../../interfaces';

import { str } from '../../utils/crc32';
import {
...
export default class Full implements TransportProtocol {
  ...
  wrap(payload: MessagePlain | MessageEncrypted): Hex {
    ...
    const crc = new Hex(str(data.toRawString()).toString(16)).toFixedBytes(4);
  ...
  unWrap(data: Hex): MessagePlain | MessageEncrypted | null {
    ...
    const crc = envelopedData.view.getNumber(tlen - 4, 4);
    ...
    if (crc !== str(dataWithoutCRC.toRawString())) throw new Error('Invalid CRC for incoming message');
