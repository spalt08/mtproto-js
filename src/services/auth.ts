...
import { Transport, DataStorage } from '../interfaces';
import TypeLanguage from '../tl';
import { Hex, MessageData, MessageHeaders } from '../serialization';
...
export default class AuthService {
  ...
  async prepare() {
    ...
    } else if (this.keys.temporary.expires < second || this.keys.temporary.key.length === 0) {
  ...
  async loadFromStorage() {
    ...
    this.keys = {
      temporary: {
        key: tempKey,
        id: tempKey.length > 0 ? SHA1.Hex(tempKey).sliceBytes(-8) : new Hex(),
        expires: expireAt || 0,
        binded: isBinded || false,
      },
      permanent: {
        key: permKey,
        id: tempKey.length > 0 ? SHA1.Hex(permKey).sliceBytes(-8) : new Hex(),
      },
    };
  }
  ...
  async createAuthKey(isTemporary: boolean = false, expiresAfter: number = 3600 * 24 * 7) {
    ...
    const ga = BigInt(serverDH.params.g_a.hex.toString(), 16);
    const dhPrime = BigInt(serverDH.params.dh_prime.hex.toString(), 16);
    ...
    if (isTemporary) {
      ...
      this.tempKeyExpire = second + expiresAfter;
    } else {
  ...
  async bindTempAuthKey() {
    ...
    const { result } = await this.transport.call(query, ({ msgID } as MessageHeaders));
